  ...
  return (
    <section className="py-12 sm:py-28 px-5 sm:px-6">
      <div className="max-w-5xl mx-auto">
        <ScrollReveal>
          <h2 className="text-xl sm:text-3xl lg:text-4xl font-extrabold tracking-tight text-center mb-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 mt-8 sm:mt-12">
          {spaces.map((space, i) => (
            <ScrollReveal key={space.name} delay={i * 0.08}>
              <div className="group p-4 sm:p-5 rounded-xl bg-card border border-border hover:border-muted-foreground/30 hover:shadow-[0_0_20px_hsl(0_0%_100%_/_0.06)] transition-all duration-300 h-full">
                <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-gradient-to-br from-foreground/20 to-foreground/5 flex items-center justify-center mb-3 sm:mb-4">
                  <space.icon className="w-7 h-7 text-foreground" />
                </div>
                <h3 className="font-semibold text-base sm:text-lg mb-1.5 sm:mb-2">{space.name}</h3>
