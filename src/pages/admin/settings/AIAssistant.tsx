...
export default function AIAssistant() {
  ...
  const handleSaveSettings = () => {
    ...
    const topPToSave = config.model?.startsWith('openai/') ? 1 : config.top_p;
    updateMutation.mutate({
      model: config.model,
          ...
          <TabsContent value="settings" className="space-y-4">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Controla a diversidade do vocabulário (0.1 = vocabulário restrito, 1.0 = vocabulário amplo)
                </CardDescription>
                {isOpenAIModel && (
                  <p className="text-sm text-muted-foreground mt-1">
                    ⚠️ Modelos OpenAI não suportam ajuste de top_p
                  </p>
                )}
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex items-center gap-4 max-w-md">
                  <Slider
                    value={[isOpenAIModel ? 1 : (config.top_p ?? 0.9)]}
                    onValueChange={([value]) => {
                      if (!isOpenAIModel) {
                        setConfig({ ...config, top_p: value });
                      }
                    }}
                    min={0.05}
                    max={1}
                  ...
                  />
                  <span className="text-sm font-mono w-12 text-right">
                    {(isOpenAIModel ? 1 : (config.top_p ?? 0.9)).toFixed(2)}
                  </span>
                </div>
              </CardContent>
            </Card>
