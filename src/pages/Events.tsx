...
function EventCard({ event, isPurchased }: { event: Event; isPurchased: boolean }) {
  ...
  return (
    <div className={`bg-card rounded-xl overflow-hidden transition-opacity ${isPast ? "opacity-60" : ""}`}>
            ...
            {pastEvents.length > 0 && (
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-semibold text-foreground">Eventos Anteriores</h2>
                  <span className="text-xs text-muted-foreground bg-secondary px-2 py-0.5 rounded-full">
                    {pastEvents.length} realizado{pastEvents.length !== 1 ? "s" : ""}
                  </span>
                </div>
