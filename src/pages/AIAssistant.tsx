...
const SUGGESTIONS = ["O que a plataforma Subhumano oferece?", "Quem está falando sobre IA para vídeos?", "Qual IA devo usar para código?", "Resuma os posts recentes sobre produtividade"];
const CAPABILITIES = [{
  icon: Sparkle,
  text: "Comparar modelos de IA (preços, recursos, limitações)"
}, {
  icon: Lightbulb,
  text: "Explicar conceitos de forma acessível"
}, {
  icon: BookOpen,
  text: "Recomendar ferramentas para seu caso"
}, {
  icon: FileText,
  text: "Gerar prompts otimizados"
}, {
  icon: Users,
  text: "Indicar discussões relevantes na comunidade"
}, {
  icon: ChatsCircle,
  text: "Resumir posts publicados na plataforma"
}];
export default function AIAssistant() {
  ...
  const markdownComponents = {
    p: ({
      children
    }: any) => <p className="mb-2 last:mb-0">{children}</p>,
    ul: ({
      children
    }: any) => <ul className="list-disc ml-4 mb-2">{children}</ul>,
    ol: ({
      children
    }: any) => <ol className="list-decimal ml-4 mb-2">{children}</ol>,
    li: ({
      children
    }: any) => <li className="mb-1">{children}</li>,
    code: ({
      children,
      className
    }: any) => {
    ...
    pre: ({
      children
    }: any) => <pre className="bg-background p-3 rounded-lg overflow-x-auto mb-2">{children}</pre>,
    h1: ({
      children
    }: any) => <h1 className="text-lg font-bold mb-2">{children}</h1>,
    h2: ({
      children
    }: any) => <h2 className="text-base font-bold mb-2">{children}</h2>,
    h3: ({
      children
    }: any) => <h3 className="text-sm font-bold mb-2">{children}</h3>,
    strong: ({
      children
    }: any) => <strong className="font-bold">{children}</strong>,
    em: ({
      children
    }: any) => <em className="italic">{children}</em>,
    a: ({ href, children }: any) => {
      ...
      if (resolvedHref?.startsWith('/')) {
        return <button onClick={e => { e.preventDefault(); navigate(resolvedHref); }}
          className="underline text-primary hover:text-primary/80 cursor-pointer">
            {children}
          </button>;
      }
      return <a href={resolvedHref} target="_blank" rel="noopener noreferrer"
          ...
          {messages.length === 0 ? <div className="flex flex-col items-center gap-6 pt-6">
              ...
              <div className="w-full max-w-sm bg-card rounded-xl p-4">
                ...
                <ul className="space-y-2">
                  {CAPABILITIES.map((cap, idx) => <li key={idx} className="flex items-start gap-2 text-sm text-muted-foreground">
              ...
              {messages.map((message, index) => <div key={index} className={cn("flex", message.role === "user" ? "justify-end" : "justify-start")}>
                  <div className={cn("max-w-[85%] rounded-2xl px-4 py-3 relative group", message.role === "user" ? "bg-card text-foreground" : "bg-secondary text-foreground")}>
                    {message.role === "assistant" && <>
                        ...
                        {message.content && <button onClick={() => handleCopyMessage(message.content, index)} className="absolute -bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity bg-elevated rounded-full p-1.5 hover:bg-border" title="Copiar resposta">
                            {copiedIndex === index ? <Check className="w-3.5 h-3.5 text-accent" /> : <Copy className="w-3.5 h-3.5 text-muted-foreground" />}
                          </button>}
                      </>}
