...
function isAllZero(stats: Omit<StatsRow, "id" | "updated_at">): boolean {
  return (
    stats.total_likes === 0 &&
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: existing, error: existingError } = await supabase
      .from("platform_stats")
      .select("id, total_likes, total_comments, total_saves, total_members, total_articles, total_podcasts, total_posts, updated_at")
    ...
    const shouldRecompute =
      ...
      isAllZero({
        ...
        total_saves: existing.total_saves,
        total_members: existing.total_members,
        total_articles: existing.total_articles,
        total_podcasts: existing.total_podcasts,
        total_posts: existing.total_posts,
      }) ||
      isStale(existing.updated_at);

    let payload: Omit<StatsRow, "id">;
    ...
    const message = error instanceof Error ? error.message : "Erro inesperado ao carregar estatísticas";
