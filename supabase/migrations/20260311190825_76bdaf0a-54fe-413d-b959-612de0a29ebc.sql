CREATE OR REPLACE FUNCTION public.search_rag_chunks_lexical(query_text text, match_count integer DEFAULT 10, filter_tags text[] DEFAULT NULL::text[], filter_layer text DEFAULT NULL::text, include_constitution boolean DEFAULT true)
 RETURNS TABLE(id uuid, document_id uuid, document_title text, layer text, content text, priority integer, tags text[], rank real)
 LANGUAGE plpgsql
 SECURITY DEFINER
